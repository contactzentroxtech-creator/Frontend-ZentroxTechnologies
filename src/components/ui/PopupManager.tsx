    ...
    <AnimatePresence>
      {activePopup && (
        <>
          ...
          >
            <div className="relative mx-4 w-full max-w-md overflow-hidden rounded-3xl border border-[#F0E6D8] bg-white p-8 shadow-2xl shadow-slate-900/10">
              <div className="pointer-events-none absolute -right-20 -top-20 h-56 w-56 rounded-full bg-blue-500/10 blur-[80px]" />

              <button
                onClick={() => dismiss(activePopup)}
                className="absolute right-4 top-4 rounded-full p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-900"
                aria-label="Close popup"
              >
                <X size={18} />
              </button>

              <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-blue-200/60 bg-gradient-to-br from-blue-50 to-indigo-50">
                {(() => {
                  const meta = POPUP_META[activePopup.type as keyof typeof POPUP_META] || POPUP_META.default;
                  const Icon = meta.icon;
                  return <Icon size={26} className="text-blue-600" />;
                })()}
              </div>

              <div className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-blue-200/60 bg-blue-50/80 px-3.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-blue-700">
                {(() => {
                  const meta = POPUP_META[activePopup.type as keyof typeof POPUP_META] || POPUP_META.default;
                  return meta.badge;
                })()}
              ...
              {activePopup.ctaText && activePopup.ctaLink && (
                <Link
                  ...
                  onClick={() => dismiss(activePopup)}
                  className="group mb-3 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-600/30"
                >
                  {activePopup.ctaText}
              ...
              <button
                onClick={() => dismiss(activePopup)}
                className="w-full py-1 text-center text-sm font-medium text-slate-500 transition-colors hover:text-slate-900"
              >
