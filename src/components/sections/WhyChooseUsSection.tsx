...
function ReasonCard({ reason, index }: { reason: (typeof REASONS)[number]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="h-full rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:border-blue-300 hover:shadow-md">
...
export default function WhyChooseUsSection() {
  ...
  return (
    <section id="why-choose-us" className="bg-white px-4 py-16 sm:py-20 md:px-6 md:py-24 lg:py-28">
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto mb-12 max-w-3xl text-center sm:mb-14">
          <span className="text-xs font-medium uppercase tracking-wider text-blue-600">Why Zentrox</span>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-800 sm:text-4xl">{t("global.why.title")}</h2>
          <p className="mt-3 text-base leading-relaxed text-slate-600 sm:text-lg">{t("global.why.sub")}</p>
        </div>
        ...
        <div className="mt-10 text-center">
          <Link href="/contact" className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-6 py-3 text-sm font-medium text-white shadow-md transition-all hover:bg-blue-700 hover:shadow-lg">
