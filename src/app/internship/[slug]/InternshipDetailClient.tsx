...
import {
  Briefcase, Clock, Users, Award, CheckCircle2, ArrowRight,
  AlertCircle, X, Check, Timer, ChevronRight, MapPin, Zap
} from 'lucide-react';
...
const DOMAIN_LABELS: Record<string, string> = {
  'web-dev': 'Web Development', 'mobile': 'Mobile Development', 'ai-ml': 'AI/ML',
  'seo': 'SEO & Marketing', 'design': 'UI/UX Design', 'digital-marketing': 'Digital Marketing', 'fullstack': 'Full Stack',
};
...
export default function InternshipDetailClient({ slug }: { slug: string }) {
  ...
  const [testResult, setTestResult] = useState<{ score: number; passed: boolean; correct: number; total: number } | null>(null);
  ...
  const apply = async () => {
    ...
    try {
      ...
      if (data.requiresTest) {
        toast('You need to complete the aptitude test to proceed.', { icon: '📝' });
        loadTest();
      }
    } catch (err: any) {
  ...
  const submitTest = async () => {
    ...
    const answersArr = Object.entries(answers).map(([questionId, selectedIndex]) => ({ questionId, selectedIndex }));
    try {
      const { data } = await api.post(`/internship/${internship!._id}/test/submit`, {
        testId, answers: answersArr, timeTaken,
      });
  ...
  if (!internship) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <Link href="/internship" className="text-z-accent hover:underline text-sm">View all internships</Link>
    </div>
  );
    ...
    <section className="relative z-10 py-16 px-4 md:px-6">
      <div className="max-w-5xl mx-auto">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                {testResult ? (
                  ...
                  <div className="text-center py-6">
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 ${testResult.passed ? 'bg-z-accent3/20 border border-z-accent3/30' : 'bg-red-500/20 border border-red-500/30'}`}>
                      {testResult.passed ? <CheckCircle2 size={36} className="text-z-accent3" /> : <X size={36} className="text-red-400" />}
                    </div>
                    <h2 className="text-2xl font-extrabold text-white mb-2">
                      {testResult.passed ? 'You Passed!' : 'Better Luck Next Time'}
                    </h2>
                    <div className="text-5xl font-extrabold text-z-accent mb-2">{testResult.score}%</div>
                    <p className="text-z-muted text-sm mb-2">{testResult.correct} / {testResult.total} correct answers</p>
                    ...
                    <button onClick={() => { setShowTest(false); }} className="px-8 py-3 rounded-xl bg-z-accent text-white font-semibold text-sm hover:bg-blue-500 transition-colors">
                      {testResult.passed ? 'Back to Internship' : 'View Other Internships'}
                    </button>
                  </div>
                ) : (
                  /* Active Test */
                  <>
                    <div className="flex items-center justify-between mb-5">
                      <div>
                        <h2 className="text-base font-bold text-white">Aptitude Test</h2>
                        <p className="text-xs text-z-muted">{internship.title} — Question {testCurrent + 1} of {testQuestions.length}</p>
                      </div>
                      <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-bold ${timeLeft < 120 ? 'text-red-400 bg-red-500/15' : 'text-z-accent bg-z-accent/15'}`}>
                        <Timer size={14} />
                        {formatTime(timeLeft)}
                      </div>
                    </div>
                    ...
                    <div className="h-1.5 rounded-full bg-white/10 mb-5 overflow-hidden">
                      <div className="h-full bg-z-accent rounded-full transition-all duration-300"
                        style={{ width: `${((testCurrent + 1) / testQuestions.length) * 100}%` }} />
                    </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-2 mb-6">
                          {testQuestions[testCurrent].options.map((opt, i) => (
                            <button key={i} onClick={() => setAnswers(prev => ({ ...prev, [testQuestions[testCurrent]._id]: i }))}
                              ...
                              <span className="font-semibold mr-2 text-z-muted">{String.fromCharCode(65 + i)}.</span>
                              {opt}
                            </button>
                          ))}
                        </div>
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => setTestCurrent(Math.max(0, testCurrent - 1))} disabled={testCurrent === 0}
                          ...
                          {testCurrent < testQuestions.length - 1 ? (
                            <button onClick={() => setTestCurrent(testCurrent + 1)}
                            ...
                            <button onClick={submitTest} disabled={testSubmitting}
                              ...
                              {testSubmitting ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Check size={14} /> Submit Test</>}
                            </button>
                          )}
                        </div>
                      </div>
        ...
        <div className="flex items-start gap-3 mb-2">
          <Link href="/internship" className="text-xs text-z-muted hover:text-white transition-colors">Internships</Link>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="z-badge mb-4">
                ...
                Remote Internship · {DOMAIN_LABELS[internship.domain] || internship.domain}
              </div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight mb-4">{internship.title}</h1>
              <div className="flex flex-wrap gap-3 mb-5">
                {[
                  { icon: Clock, val: internship.duration },
                  { icon: MapPin, val: internship.type === 'remote' ? 'Fully Remote' : 'Hybrid' },
                  { icon: Users, val: `${slotsLeft} slots left` },
                ].map(({ icon: Icon, val }) => (
                  <div key={val} className="flex items-center gap-1.5 text-sm text-z-muted">
                    <Icon size={14} className="text-z-accent" /> {val}
                  </div>
                ))}
              </div>
              <p className="text-base text-z-muted leading-relaxed">{internship.description}</p>
            </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                <ul className="flex flex-col gap-2.5">
                  {internship.requirements.map((req, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-sm text-z-muted">
                      <CheckCircle2 size={15} className="text-z-accent3 flex-shrink-0 mt-0.5" /> {req}
                    </li>
                  ))}
                </ul>
              </motion.div>
            ...
            {internship.tasks?.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                ...
                <div className="flex flex-col gap-3">
                  {internship.tasks.sort((a, b) => a.order - b.order).map((task, i) => (
                    <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-white/5 border border-z-border">
                      ...
                      <div>
                        ...
                        {task.description && <div className="text-xs text-z-muted mt-0.5 leading-relaxed">{task.description}</div>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { show: internship.offerLetter, icon: '📄', title: 'Offer Letter', desc: 'On acceptance' },
                  { show: internship.certificate, icon: '🏆', title: 'Completion Certificate', desc: 'Verified + QR code' },
                  { show: true, icon: '⭐', title: 'Recommendation Letter', desc: 'Performance-based' },
                  { show: true, icon: '🚀', title: 'Real-world Experience', desc: 'Industry projects' },
                ].filter(i => i.show).map(item => (
                  <div key={item.title} className="p-4 rounded-xl bg-white/5 border border-z-border">
                    <div className="text-xl mb-2">{item.icon}</div>
                    <div className="text-sm font-semibold text-white">{item.title}</div>
                    <div className="text-xs text-z-muted mt-0.5">{item.desc}</div>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              ...
              <div className="flex flex-col gap-2 mb-5 text-sm">
                {[
                  ...
                  ['Domain', DOMAIN_LABELS[internship.domain] || internship.domain],
                  ['Aptitude Test', internship.aptitudeTestRequired ? `Required (min ${internship.minAptitudeScore}%)` : 'Not required'],
                  ['Certificate', internship.certificate ? 'Included' : 'Not included'],
                ].map(([k, v]) => (
              ...
              {applied ? (
                <div className="flex flex-col gap-3">
                  <div className={`flex items-center gap-2 p-3 rounded-xl text-sm font-semibold ${
                    applicationStatus === 'test-passed' ? 'bg-z-accent3/15 text-z-accent3' :
                    applicationStatus === 'test-failed' ? 'bg-red-500/15 text-red-400' :
                    'bg-z-accent/15 text-z-accent'
                  }`}>
                    <CheckCircle2 size={15} />
                    {applicationStatus === 'test-passed' ? 'Test passed — under review' :
                     applicationStatus === 'test-failed' ? 'Test not passed' :
                     'Applied — test required'}
                  </div>
                  {applicationStatus === 'pending' && internship.aptitudeTestRequired && (
                ...
                <button onClick={apply} disabled={applying || slotsLeft <= 0 || !user}
                  ...
                  {applying ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> :
                   slotsLeft <= 0 ? 'No Seats Available' :
                   !user ? 'Login to Apply' :
